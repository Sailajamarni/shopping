...
public class CheckAvailabilityController extends HttpServlet {
    ...
    boolean isPincodeServiceable(int pincode, int category) {
        ...
        try {
            ...
            stmt = conn.prepareStatement("SELECT COUNT(*) FROM ServiceableRegions_1 sr JOIN ProductCategoryWiseServiceableRegions_1 pr ON sr.srrg_id = pr.srrg_id WHERE ? BETWEEN sr.srrg_pinfrom AND sr.srrg_pinto AND pr.prct_id = ?");
